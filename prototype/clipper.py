...
clipper.py
==========
Video Clipper and Labeler GUI for the Paryamiel/prototype 5-class exam behavior
dataset.

Adapted from IntegraPose (https://github.com/farhanaugustine/IntegraPose)
  - Source: Tandem YOLO-LSTM Classifier/gui/clipper.py
  - Commit: eb644f118be0ad89b457d933bccf47ede5ba9821
  - License: AGPL-3.0 (see https://www.gnu.org/licenses/agpl-3.0.en.html)
  - Paper:  Augustine et al. (2025), Integrapose: A Unified Framework for
           Simultaneous Pose Estimation and Behavior Classification.
           https://doi.org/10.5281/zenodo.15565090

Modifications from the original:
  1. Config path can be passed via --config CLI arg (defaults to
     ./clipper_config.json next to this script).
  2. Removed dependency on the parent project structure; this file is fully
     standalone.
  3. Added attribution header (this block).
...
Workflow
--------
1. Click "Load Video" and select a long recording (.mp4 / .avi / .mov).
2. Use the slider or the << / >> buttons to scrub to the start of a behavior.
3. Press one of the labeling keys to pick a class:
       n = normal
       h = hand_signal
       p = passing_of_notes
       s = side_glancing
       u = use_of_unauthorized_object
4. Click "Set Start Frame" at the segment start.
5. Scrub forward to the segment end, click "Set End Frame".
6. Click "Add Clip to Queue" to enqueue the (behavior, start, end) tuple.
7. Repeat steps 2-6 for every behavior segment in this recording.
8. Click "Extract All Queued Clips" to batch-export all queued segments as
   individual .mp4 files into <source_directory>/<behavior>/.

Output filenames follow the IntegraPose convention:
    <video_stem>_<Behavior>_frames_<start>_<end>.mp4

The downstream extract_dataset.py in this repo reads .mp4 files from
data/raw_videos/<class>/ folders, so clipper output lands in the right place
automatically.
"""
import argparse
import json
...
class VideoClipperApp:
    ...
    def __init__(self, root: tk.Tk, config: Dict[str, Any]):
        ...
        """
        self.root = root
        self.root.title("Exam Behavior Video Clipper and Labeler (5-Class)")
        ...
        os.makedirs(self.clip_save_dir, exist_ok=True)
    ...
    def _setup_gui(self):
        ...
        self.lbl_selected_behavior = tk.Label(
            label_frame, text="Behavior: None", font=("Helvetica", 10, "bold"), fg="green"
        )
        ...
        tk.Button(
            queue_buttons, text="Add Clip to Queue", command=self.add_clip_to_queue
        ).pack(side=tk.LEFT, expand=True, fill=tk.X, padx=2)
        tk.Button(
            queue_buttons, text="Remove Selected", command=self.remove_selected_clip
        ).pack(side=tk.LEFT, expand=True, fill=tk.X, padx=2)
        ...
        self.btn_extract = tk.Button(
            action_frame,
            text="Extract All Queued Clips",
            command=self.extract_all_clips,
            bg="#22c55e",
            fg="white",
        ...
        self.lbl_status = tk.Label(
            self.root, text="Ready. Load a video to begin.", bd=1, relief=tk.SUNKEN, anchor=tk.W
        )
    ...
    def extract_all_clips(self):
        ...
        for i, clip_data in enumerate(self.clips_to_save):
            ...
            self.lbl_status.config(
                text=f"Extracting clip {i+1}/{len(self.clips_to_save)} ('{behavior}')..."
            )
            self.root.update_idletasks()
...
def launch_clipper_gui(config_path: str = "clipper_config.json"):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Exam Behavior Video Clipper (5-Class)")
    parser.add_argument(
        "--config",
        default="clipper_config.json",
        help="Path to clipper_config.json (default: ./clipper_config.json)",
    )
    args = parser.parse_args()
    launch_clipper_gui(args.config)